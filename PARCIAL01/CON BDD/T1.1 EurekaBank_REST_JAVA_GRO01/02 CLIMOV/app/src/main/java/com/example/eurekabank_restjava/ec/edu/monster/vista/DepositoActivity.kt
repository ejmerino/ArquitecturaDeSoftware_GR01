...
class DepositoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnOk.setOnClickListener { hacerDeposito() }
    }

    private fun hacerDeposito() {
        ...
        ui.launch {
            try {
                val resp = withContext(Dispatchers.IO) {
                    Repo.deposito(cuenta, importe)
                }
                if (resp.isSuccessful) {
                    Toast.makeText(this@DepositoActivity, "Depósito realizado", Toast.LENGTH_LONG).show()
                    txtImporte.setText("")
                } else {
                    Toast.makeText(
                        this@DepositoActivity,
                        "Error ${resp.code()}: ${resp.message()}",
                        Toast.LENGTH_LONG
                    ).show()
                }
            } catch (e: Exception) {
                Toast.makeText(this@DepositoActivity, "Fallo: ${e.message}", Toast.LENGTH_LONG).show()
                e.printStackTrace()
            }
        }
    }
